...
## v2-pipeline kills (added during 50-run pass)

- **v2-S3-R009-001 / F2-CKDJ — Softmax shift-invariance strip via static outlier channels × W_Q column energy** (arch-transpose, frame-novelty). REGENERATED 2026-05-09 by run_009 Stage 3. CF9 failure: the static-channel query offset δq_c contributes δq_c^T k_t to logit_t, which varies per key position t — this is NOT a constant scalar per attention-row and cannot be stripped losslessly by softmax shift-invariance. The "zero-cost lossless strip" claim is broken. Surviving primitive: static outlier channels (CF3 K=0.1%) as high-energy W_Q input columns — measurement claim useful for W_Q quantization precision allocation, ADJACENT to SageAttention2 (arXiv:2411.10958) and FireQ (arXiv:2505.20839). Pass to Stage 4: measure whether CF3 static channels = top W_Q column energy channels; if YES, motivates per-column precision allocation. Not a Stage 5 target this round as a standalone mechanism.

- **v2-S3-R008-001 / F4-SVAL-CONSERVED — Sinkhorn gauge-exploitation for spectral rank prediction** (gauge-exploitation, elegant-equivalence). KILLED 2026-05-09 by run_008 Stage 3. PRE-EMPTED by SINQ (arXiv:2509.22944, Sep 2025, updated Feb 2026): Sinkhorn-normalized quantization using identical per-row/column Sinkhorn-Knopp iterations to minimize weight matrix imbalance, evaluated on Qwen3 family, integrated into Hugging Face Transformers. F4's core mechanism "derive optimal compression gauge from Sinkhorn balance to minimize spectral entropy" is precisely what SINQ publishes for the quantization use case. ≥ 2/3 load-bearing claims PRE-EMPTED. Surviving primitive: SINQ's Sinkhorn balance could be applied to predict SVD-safe truncation rank (not just quantization scale) — a minor extension that does not warrant a Stage 5 slot this round.

- **v2-S3-R007-003 / U4-A IOPB — IOCP token budget as attention-head bandwidth regulator** (systems-os, arch-transpose). REGENERATED 2026-05-09 by run_007 Stage 3. CF9 partial failure: IOCP `NumberOfConcurrentThreads` throttles completion-processing threads, NOT NVMe I/O throughput; the NVMe SSD services I/O requests at hardware speed regardless of IOCP completion concurrency setting. "Bandwidth governor" claim is broken by IOCP semantics. Surviving primitive: IOCP completion ordering as computation serializer for per-head scheduling (adjacent to standard semaphore-gated pipeline, not novel frame). Pass to Stage 4 as narrow engineering experiment (3-hour harness; go = 5% tok/s improvement from reduced DRAM contention). Not a Stage 5 target this round.

- **v2-S3-R007-002 / F7-1 WOVR — W_O left-singular / W_V right-singular spectral coupling** (compression-lr, arch-fold). DOWNGRADED 2026-05-09 by run_007 Stage 3. Engineering compression application pre-empted by A3 (arXiv:2505.12942, May 2025): A3 explicitly computes the fused W_vo = W_v·W_o product and applies activation-aware SVD compression at 70B scale with SOTA perplexity 4.69 on WikiText-2. Structural science question (principal-angle alignment U_O ≈ V_V as SGD attractor) is NOVEL but below Stage 5 selection threshold — it is a 3-min add-on measurement to the ATRC W_V spectrum sweep, not a standalone experiment. Not a Stage 5 target this round.

- **v2-S3-R007-001 / C8-WDOS — W_down output-space × tied-embed disjointness** (compression-subspace). KILLED 2026-05-09 by run_007 Stage 3. CF12 establishes E is nearly isotropic (r_99=1992/2048, no dominant directions). The "E^T-invisible subspace" is bounded to ≤ (2048-1992)/2048 ≈ 2.7% of W_down output energy — payoff null. Compressing W_down based on disjointness from the 56 non-span directions of E^T gives at most 2.7% of output energy in the safe zone; negligible practical gain. Mechanism premise refuted by CF12 arithmetic.

- **v2-S3-R004-001 / F-GFQO — Gauge-freedom general residual-stream rotation** (arch-transpose, frame-novelty). DOWNGRADED 2026-05-09 by run_004 Stage 3. CF9 partial failure: general orthogonal rotation R is incompatible with RMSNorm per-channel scale γ (the group of matrices commuting with any diagonal is the generalized permutation group — sign flips × permutations only, not O(d)). The "attention-invisible block" exposed by arbitrary R does not survive RMSNorm's per-channel scale. What survives after stripping the broken import: per-channel attention-subspace importance ordering, which is ADJACENT to SmoothQuant / activation-aware quantization (not novel). Stripped primitive: per-channel W_Q projection magnitude as a mixed-precision assignment signal → pass to Stage 4 as quantization-alignment mechanism, not standalone compression. Not a Stage 5 target this round.

- **v2-CHEAP-TEST-001 / Cluster C1 — Cross-layer W_Q subspace sharing class** (compression-lr, arch-transpose). KILLED 2026-05-09 by run_001 cheap-test (F-CQSGC). The matched-spectrum random-orientation baseline (Stage 6 blocking amendment A2) caught the false-positive: real stacked var@128 = 0.2560 vs permuted-rows var@128 = 0.2559 (gap = 0.0001). The "cross-layer subspace coherence" was entirely the within-layer CF11 concentration trivially summed in the stack. W_Q subspaces rotate independently across all 28 layers. **Class kill**: kills R-CROSS-Q, C-CQBAL, F-CQSGC, A-GFRS (run 001 Cluster C1) AND F2-CQBS, R3-XLQB, C2-CQST, A1-GHPI (run 002 Cluster C1) AND any future variant of "cross-layer W_Q stacked SVD / shared basis / joint diagonalization without retraining." Per-layer K=128 (CF11) remains the ceiling for attention W_Q post-training compression. Result file: `sonnet_ladder_v2/cheap_tests/run_001/fcqsgc_result.md`. This is the v2 cascade catching exactly the false-positive its skeptic-controls gate (added 2026-05-09) was designed to surface.
